...
export class DirectMessageGateway extends CoreGateway implements OnGatewayInit {
    ...
    async sendMessage(@MessageBody() {receiver, content}: SendMessageDto, @ConnectedSocket() socket: Socket) {
        const userId = socket['user']?.id;
        const {chat, message} = await this.directMessageService.sendMessage(content, userId, receiver);
        this._server.to(this._connectedSockets.get(receiver)).emit('receivedDirectMessage', {message, chat});
